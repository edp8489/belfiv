...
C       > BELFIV -F TELLS PROGRAM TO USE INPUT FILE, GO TO LINE 220
      INCLUDE 'C:\WATCOM\SRC\FORTRAN\FSUBLIB.FI'
      NARG = IARGC()
C      WRITE(6,2) NARG
C    2 FORMAT(1X,'NARGS IS ',I2)
C      PRINT '(A,I2)', 'NARGS IS', IARGC
C      IF(NARG .EQ. 1) GO TO 20
      IF (NARG .EQ. 2) THEN
          CALL IGETARG(1, CARG)
          IF (CARG .EQ. 'K') THEN
              GO TO 30
      ...
      END IF
C   10 CONTINUE
   15 FORMAT(1X,'ERROR WITH ARGUMENTS. TRY AGAIN'/
     $'(-K) FOR KEYBOARD INPUT OR (-F) FOR FILE INPUT')
